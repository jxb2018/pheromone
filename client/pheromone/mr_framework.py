...
class MR:
    ...
    def map_reduce(self, map_function, reduce_function, map_inputs, kvs_input=False, reduce_num=2, custom_key=None,
        ...
        self._change_variables_in_file(os.path.join(self.workspace, 'mapper_template.cpp'),
                                       os.path.join(self.workspace, 'mapper.cpp'), map_vars)
        ...
        self._change_variables_in_file(os.path.join(self.workspace, 'reducer_template.cpp'),
                                       os.path.join(self.workspace, 'reducer.cpp'), reduce_vars)

        # build .so
        map_func_name = map_function.split('.')[0]
        reduce_func_name = reduce_function.split('.')[0]
        print('Compiling the mapper and reducer...')
        command = subprocess.run([f'cd {self.workspace} && bash compile_upload.sh {map_func_name} {reduce_func_name}'],
                                 shell=True)
        ...
        if res:
            res = self.client.add_trigger(app_name, bucket_name, f't_{app_name}', DYNAMIC_GROUP,
                                          {'function': reduce_func_name,
                                           'control_group': [f'ctrl:{i}' for i in range(len(map_inputs))]},
                                          trigger_option=1)
