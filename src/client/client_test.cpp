...
inline pair<string, unsigned>
get_coord(string &app_name, map<string, pair<string, unsigned>> &app_coord_map, zmq::socket_t &mngt_socket) {
...
//    unsigned req_num = conf["req"].as<unsigned>();
    ...
    for (int i = 0; i < app_num; i++) {
        ...
        kZmqUtil->send_string(serialized, &pushers["tcp://" + coord_thread.first + ":" +
