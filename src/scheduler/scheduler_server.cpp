...
auto rerun_check_queue_compare = [](RerunCheckQueueItem first, RerunCheckQueueItem second) {
    return first.check_time_ > second.check_time_;
};
...
void forward_call_via_helper(CommHelperInterface *helper, string &session_id, string &app_name,
    ...
    if (arg_flag > 0) {
        ...
        if (total_data_size < forwardDataPackingThreshold) {
            ...
            helper->forward_func_call(session_client_addr_map[session_id], app_name, func_name_vec, data_args_vec, 0,
                                      session_id);
        } else {
            helper->forward_func_call(session_client_addr_map[session_id], app_name, func_name_vec, func_args_vec,
                                      arg_flag, session_id);
        }
    } else {
        helper->forward_func_call(session_client_addr_map[session_id], app_name, func_name_vec, func_args_vec, arg_flag,
                                  session_id);
    }
}

void schedule_func_call(logger log, CommHelperInterface *helper, map<uint8_t, uint8_t> &executor_status_map,
                        map<string, set<uint8_t>> &function_executor_map,
                        string &session_id, string &app_name, string &func_name, vector<string> &func_args,
                        int arg_flag) {
    vector<uint8_t> avail_warm_executors;
    vector<uint8_t> avail_cold_executors;
    for (auto &executor_status: executor_status_map) {
        if (executor_status.second == 1 &&
            function_executor_map[func_name].find(executor_status.first) != function_executor_map[func_name].end()) {
...
//                  executor_id, sched_stamp);
        ...
        if (schedDelayTime > 0) {
            delay_call_queue.push(DelayFunctionCall{session_id, app_name, func_name, func_args, arg_flag,
...
vector<string> check_trigger(logger log, BucketKey &bucket_key, string &session_id,
                             CommHelperInterface *helper, map<Bucket, vector<TriggerPointer>> &bucket_triggers_map,
                             map<string, set<uint8_t>> &function_executor_map,
    ...
    for (auto &func_metadata: active_func_metadata) {
        schedule_func_call(log, helper, executor_status_map, function_executor_map, session_id,
                           bucket_app_map[bucket_key.bucket_], func_metadata.func_name_, func_metadata.func_args_,
                           func_metadata.arg_flag_);
    }
                ...
                if (from_ephe_store) {
                    if (key_len_map.find(key_name) != key_len_map.end()) {
                        ...
                        auto ready_time = std::chrono::duration_cast<std::chrono::microseconds>(
                                ready_stamp.time_since_epoch()).count();
            ...
            if (str[1] >= 2 && str[1] <= 4) {
                ...
                if (infos.size() < 6) {
                    ...
                    if (str[1] == 2) {
                        ...
                        if (app_info_map[app_name].direct_deps_.find(src_function) !=
                            app_info_map[app_name].direct_deps_.end()) {
                            ...
                            if (tgt_function.empty()) {
                                ...
                                if (target_funcs.size() > avail_executors) {
                                    vector<string> local_funcs(target_funcs.begin(),
                                                               target_funcs.begin() + avail_executors);
                                    vector<string> remote_funcs(target_funcs.begin() + avail_executors,
                                                                target_funcs.end());
                                    for (auto &func: local_funcs)
                                        schedule_func_call(log, helper, executor_status_map, function_executor_map,
                                                           session_id, app_name, func, func_args, arg_flag);
                                    if (!rejectExtraReq) {
                                        vector<vector<string>> func_args_vec;
                                        for (int i = 0; i < remote_funcs.size(); i++)
                                            func_args_vec.push_back(func_args);
                                        forward_call_via_helper(helper, session_id, app_name, remote_funcs,
                                                                func_args_vec, arg_flag);
                                    }
                                } else {
                            ...
                            } else if (app_info_map[app_name].direct_deps_[src_function].find(tgt_function) !=
                                       app_info_map[app_name].direct_deps_[src_function].end()) {
                                schedule_func_call(log, helper, executor_status_map, function_executor_map, session_id,
                                                   app_name, tgt_function, func_args, arg_flag);
                            } else {
                                log->warn("Direct Invocation without pre-defined dependency {} -> {}", src_function,
                                          tgt_function);
                            }
                        }
                            // try trigger check
                            ...
                            auto active_triggers = check_trigger(log, bucket_key, session_id, helper,
                                                                 bucket_triggers_map, function_executor_map,
                                                                 executor_status_map, bucket_app_map);
                            if (is_data_packing == 1) {
                                helper->notify_put(bucket_key, active_triggers, session_client_addr_map[session_id],
                                                   key_val_map[obj_name]);
                            } else if (is_data_packing == 2) {
                                helper->notify_put(bucket_key, active_triggers, session_client_addr_map[session_id]);
                            }
//                            log->info("Notified data {}", obj_name);
                        ...
                        helper->client_response(session_client_addr_map[session_id], func_app_map[src_function],
                                                output_data);
...
//                        log->info("Check and fetch function args at: {}", check_name);
                        ...
                        if (inflight_args.inflight_args_.empty()) {
                            ...
                            schedule_func_call(log, helper, executor_status_map, function_executor_map,
                                               comm_resp.session_id_, comm_resp.app_name_, inflight_args.func_name_,
                                               func_full_args, is_func_arg_key);
                        } else {
                            call_id_inflight_args_map[call_id] = inflight_args;
                        }
                    } else {
                        schedule_func_call(log, helper, executor_status_map, function_executor_map,
                                           comm_resp.session_id_, comm_resp.app_name_, func_name, func_args, 0);
                    }
                    call_id++;
                }
                ...
                if (key_remote_get_map.find(comm_resp.data_key_) != key_remote_get_map.end()) {
                    for (auto &inflight_get_req: key_remote_get_map[comm_resp.data_key_]) {
                        ...
                        auto recv_stamp = std::chrono::duration_cast<std::chrono::microseconds>(
        ...
        if (!delay_call_queue.empty()) {
            ...
            while (cur_avail_executors > 0 && !delay_call_queue.empty()) {
                ...
                schedule_func_call(log, helper, executor_status_map, function_executor_map, delay_call.session_id_,
                                   delay_call.app_name_, delay_call.func_name_, delay_call.func_args_,
                                   delay_call.arg_flag_);
                cur_avail_executors--;
            ...
            while (!delay_call_queue.empty()) {
                ...
                auto delay_time = std::chrono::duration_cast<std::chrono::microseconds>(
                        cur_stamp - delay_call.trigger_time_).count();
