...
GetAddressResult get_address(
    ...
    if (bucket_key_address_map.find(bucket_key.bucket_) != bucket_key_address_map.end() &&
        bucket_key_address_map[bucket_key.bucket_].find(bucket_key.key_) !=
        bucket_key_address_map[bucket_key.bucket_].end()) {
...
void run(Address public_ip, Address private_ip, unsigned thread_id) {
    ...
    while (true) {
        ...
        if (pollitems[3].revents & ZMQ_POLLIN) {
            ...
            trigger_op_handler(log, serialized, private_ip, thread_id, pushers, bucket_triggers_map, bucket_app_map,
                               node_status_map);
        }
        ...
        if (pollitems[6].revents & ZMQ_POLLIN) {
            ...
            app_register_handler(log, serialized, private_ip, thread_id, pushers, bucket_triggers_map, app_buckets_map,
                                 bucket_app_map, node_status_map);
        }
