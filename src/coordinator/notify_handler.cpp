...
void notify_handler(logger log, string &serialized, SocketCache &pushers,
    ...
    for (const BucketKeyAddress &address: key_notif_request.addresses()) {
        ...
        for (auto &trigger: bucket_triggers_map[bucket_key.bucket_]) {
            ...
            if (actions.size() > 0) {
                // trigger function
                if (trigger->get_type() != PrimitiveType::BY_TIME)
                    triggered_name.push_back(trigger->get_trigger_name());

                for (auto &action: actions) {
                    auto trigger_stamp = std::chrono::duration_cast<std::chrono::microseconds>(
                            std::chrono::system_clock::now().time_since_epoch()).count();
                    ...
                    for (auto &session_key: action.session_keys_) {
                        string key_name = get_local_object_name(bucket_key.bucket_, session_key.second,
                                                                session_key.first);
                    ...
                    auto check_stamp = std::chrono::duration_cast<std::chrono::microseconds>(
                            std::chrono::system_clock::now().time_since_epoch()).count();
                    ...
                    if (all_object) {
                        ...
                        std::copy(val_to_batch.begin(), val_to_batch.end(),
                                  std::ostream_iterator<std::string>(imploded, batchDelim));
                    ...
                    auto package_stamp = std::chrono::duration_cast<std::chrono::microseconds>(
                            std::chrono::system_clock::now().time_since_epoch()).count();
