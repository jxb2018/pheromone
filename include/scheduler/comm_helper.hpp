...
enum SecondaryThreadMsgType {
    SendReq, GetKvs, PutKvs
};
...
public:
    ...
    virtual vector<RecvMsg>
    try_recv_msg(map<Bucket, vector<TriggerPointer>> &bucket_triggers_map, map<string, AppInfo> &app_info_map,
                 map<string, string> &func_app_map, map<string, string> &bucket_app_map,
                 map<string, vector<RerunTriggerTimeout>> &func_trigger_timeout_map) = 0;

    virtual void forward_func_call(string &resp_address, string &app_name, vector<string> &func_name_vec,
                                   vector<vector<string>> &func_args_vec, int arg_flag, string &session_id) = 0;
...
public:
    CommHelper(vector<HandlerThread> routing_threads, string ip, CopyFunction copy_func, GetFunction get_func,
               unsigned io_thread_count = 1, unsigned timeout = 10000) :
...
public:
            ...
            if (update_msg_type == 0) {
                ...
                for (const auto &trigger: coord_msg.triggers()) {
                    ...
                    if (!trigger_name.empty()) {
                        auto trigger_ptr = gen_trigger_pointer(trigger.primitive_type(), trigger_name,
                                                               trigger.primitive());
                        if (trigger_ptr != nullptr) {
                            ...
                            for (const auto &hint: trigger.hints()) {
                                ...
                                func_trigger_timeout_map[hint.source_function()].push_back(
                                        {trigger_ptr, hint.timeout()});
                            }
    ...
    void forward_func_call(string &resp_address, string &app_name, vector<string> &func_name_vec,
                           vector<vector<string>> &func_args_vec, int arg_flag, string &session_id) {
    ...
    void get_kvs_responses(vector<RecvMsg> &comm_resps, unsigned tid = 0) {
        ...
        for (auto &response: responses) {
            ...
            if (response.error() == AnnaError::TIMEOUT) {
                ...
                log_->info("Thread {} Kvs response type {} error code {}", tid, response.type(),
                ...
                if (response.type() == RequestType::GET && response.tuples(0).error() != 1) {
                    ...
                    auto kvs_get_stamp = std::chrono::duration_cast<std::chrono::microseconds>(
                            std::chrono::system_clock::now().time_since_epoch()).count();
                    log_->info("Kvs GET response io_thread {}. recv: {}, copy: {}.", tid, kvs_recv_stamp,
                               kvs_get_stamp);
     ...
     */
    void notify_put(const BucketKey &bucket_key, vector<string> &active_triggers, const string &resp_address,
                    const string &payload = emptyString) {
...
private:
    ...
    void handle_function_call(string &serialized, RecvMsg &resp, unsigned tid = 0) {
        ...
        log_->info("Function call {} io_thread {}. num: {}, recv: {}.", name_to_log, tid, resp.func_name_.size(),
                   receive_call_stamp);
    }
    ...
    void send_data_req_base(string &key, SocketCache &pushers, unsigned tid = 0) {
        ...
        auto send_stamp = std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::system_clock::now().time_since_epoch()).count();
        log_->info("Thread {} send data request for key {} start {} parse {} at {}.", tid, key, start_stamp,
    ...
    void notify_put_base(const BucketKey &bucket_key, vector<string> &active_triggers, const string &resp_address,
                         SocketCache &pushers, const string &payload = emptyString, unsigned tid = 0) {
